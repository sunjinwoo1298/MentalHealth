...
CREATE TABLE users (
    ...
    last_name_encrypted BYTEA,  -- Encrypted
    date_of_birth_encrypted BYTEA, -- Encrypted
    ...
    emergency_contact_encrypted BYTEA, -- Encrypted emergency contact
...
CREATE TABLE chat_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    session_type VARCHAR(50) DEFAULT 'general', -- general, crisis, wellness, professional
    status VARCHAR(20) DEFAULT 'active', -- active, ended, emergency_escalated
    mood_before INTEGER CHECK (mood_before >= 1 AND mood_before <= 10),
    mood_after INTEGER CHECK (mood_after >= 1 AND mood_after <= 10),
    session_summary TEXT,
    ai_confidence_score DECIMAL(3,2),
    intervention_triggered BOOLEAN DEFAULT FALSE,
    intervention_type VARCHAR(50), -- breathing, grounding, professional_referral, crisis_protocol
...
CREATE TABLE chat_messages (
    ...
    message_content_encrypted BYTEA NOT NULL, -- Encrypted message content
    message_type VARCHAR(30) DEFAULT 'text', -- text, image, audio, system_notification
    sentiment_score DECIMAL(3,2), -- -1 to 1 (negative to positive)
    emotion_tags TEXT[], -- anxiety, depression, stress, joy, etc.
...
CREATE TABLE interventions (
    ...
    intervention_type VARCHAR(50) NOT NULL, -- crisis_protocol, breathing_exercise, professional_referral, etc.
    trigger_reason TEXT NOT NULL,
    ai_confidence DECIMAL(3,2) NOT NULL,
    user_response VARCHAR(20), -- accepted, declined, no_response
    outcome TEXT,
...
CREATE TABLE therapists (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    email VARCHAR(255) UNIQUE NOT NULL,
    name_encrypted BYTEA NOT NULL, -- Encrypted
    license_number_encrypted BYTEA NOT NULL, -- Encrypted
...
CREATE TABLE consultations (
    ...
    referral_type VARCHAR(30) DEFAULT 'ai_suggested', -- ai_suggested, user_requested, crisis_escalation
    status VARCHAR(30) DEFAULT 'pending', -- pending, scheduled, completed, cancelled
...
CREATE TABLE user_progress (
    ...
    wellness_score DECIMAL(3,1) DEFAULT 0.0, -- 0-10 scale
...
CREATE TABLE mood_entries (
    ...
    emotions TEXT[] DEFAULT '{}', -- happy, sad, anxious, stressed, calm, etc.
...
CREATE TABLE wellness_activities (
    ...
    description TEXT,
    activity_type VARCHAR(50) NOT NULL, -- meditation, breathing, music, game, article, video
    content_url TEXT, -- For media files
    duration_minutes INTEGER,
    difficulty_level VARCHAR(20) DEFAULT 'beginner', -- beginner, intermediate, advanced
    target_emotions TEXT[], -- stress, anxiety, depression, etc.
    cultural_context VARCHAR(50), -- indian, universal, regional
...
CREATE TABLE user_activities (
    ...
    completion_status VARCHAR(30) DEFAULT 'started', -- started, completed, paused
    completion_percentage INTEGER DEFAULT 0,
    user_rating INTEGER CHECK (user_rating >= 1 AND user_rating <= 5),
    notes_encrypted BYTEA, -- Encrypted user notes
    started_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE POLICY user_data_policy ON users FOR ALL USING (id = current_setting('app.current_user_id')::UUID);
CREATE POLICY user_profile_policy ON user_profiles FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
CREATE POLICY chat_session_policy ON chat_sessions FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
CREATE POLICY chat_message_policy ON chat_messages FOR ALL USING (
    session_id IN (SELECT id FROM chat_sessions WHERE user_id = current_setting('app.current_user_id')::UUID)
);
CREATE POLICY mood_entry_policy ON mood_entries FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
CREATE POLICY user_activity_policy ON user_activities FOR ALL USING (user_id = current_setting('app.current_user_id')::UUID);
